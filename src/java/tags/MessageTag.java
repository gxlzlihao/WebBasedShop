...
public class MessageTag extends BodyTagSupport {
     ...
     */
    private void writeTagBodyContent(JspWriter out, BodyContent bodyContent) throws IOException, JspException {
        ...
        try {
            messageList = mb.reteriveMessage(user_id);

            Iterator iter = messageList.iterator();
            if(!messageList.isEmpty())
                out.println("<h3>Your Messages</h3>");
            out.println("<table bgcolor=\"silver\" cellspacing=\"0\">");
            while (iter.hasNext()) {
                out.println("<tr><td>");
